...
static bool isValidLightBlob(const std::vector<cv::Point> &contour, const cv::RotatedRect &rect){
    return (1.2 < lw_rate(rect) && lw_rate(rect) < 10) &&                 // 旋转矩形长宽比范围
           // (rect.size.area() < 3000) &&                                // 过滤画面中(不合理大小的)亮斑
           ((rect.size.area() < 50 && areaRatio(contour, rect) > 0.4) ||  // 过远的灯条会趋向于椭圆
            (rect.size.area() >= 50 && areaRatio(contour, rect) > 0.6));  // 近处的灯条轮廓更趋向于矩形
}
// 判断灯条颜色(此函数可以有性能优化).
static uint8_t get_blob_color(const cv::Mat &src, const cv::RotatedRect &blobPos) {
    auto region = blobPos.boundingRect();                // 这里为了将整个灯条囊括进去，一定程度减少灯条矩形大小受远距离成像的波动性
    region.x -= fmax(3, region.width * 0.1);             // (X方向最小填充：6个像素)
    region.y -= fmax(3, region.height * 0.05);           // (Y方向最小填充：4个像素)
    ...
    for (int row = 0; row < roi.rows; row++) { // Warning：微小矩形易因统计量导致判断失能(噪声像素占比大)
        for (int col = 0; col < roi.cols; col++) {
            red_cnt += roi.at<cv::Vec3b>(row, col)[2];
            blue_cnt += roi.at<cv::Vec3b>(row, col)[0];
        }
    }
    if (red_cnt > blue_cnt) {  // 区域特定通道像素值累加统计的方法判断灯条颜色
        return BLOB_RED;
    } else {
        return BLOB_BLUE;
    }
...
static bool isSameBlob(LightBlob blob1, LightBlob blob2) {
    ...
    return (dist.x * dist.x + dist.y * dist.y) < 9;    // 双二值化矩形重合度阈值(像素距离判断)
}

// 开闭运算 TODO: 当画面目标过大会造成处理量急速上升
