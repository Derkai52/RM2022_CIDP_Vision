...
private:
    Matrix_x1d x_k1; // k-1时刻的滤波值，即是k-1时刻的值 ----------------- 3x1矩阵
    Matrix_xzd K;    // Kalman增益 ----------------------------------- 3x1矩阵
    Matrix_xxd A;    // 转移矩阵 -------------------------------------- 3x3矩阵
    Matrix_zxd H;    // 观测矩阵 -------------------------------------- 1x3矩阵
    Matrix_xxd R;    // 预测过程噪声偏差的方差 -------------------------- 3x3矩阵
    Matrix_zzd Q;    // 测量噪声偏差，(系统搭建好以后，通过测量统计实验获得) - 1x1矩阵
    Matrix_xxd P;    // 估计误差协方差 --------------------------------- 3x3矩阵
    double last_t{0};
public:
    Kalman() = default;
