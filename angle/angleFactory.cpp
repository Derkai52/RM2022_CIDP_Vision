...
//    vertices[0] = armor_box_points[0];
//    vertices[1] = armor_box_points[1];
//    vertices[2] = armor_box_points[2];
//    vertices[3] = armor_box_points[3];
    // 建立灯条四点并排序
    static cv::Point2f vertices[4];
...
//    cv::circle(image2show, point, 15, cv::Scalar(0, 255, 0),2);//在图像中画出特征点，2是圆的半径
    ...
    // 画出各个顶点 红黄蓝绿
    cv::circle(image2show, vertices[0], 5, cv::Scalar(0, 0, 255), -1);  // 红
    cv::circle(image2show, vertices[1], 5, cv::Scalar(0, 255, 255), -1); // 黄
    cv::circle(image2show, vertices[2], 5, cv::Scalar(255, 155, 0), -1); // 蓝
    cv::circle(image2show, vertices[3], 5, cv::Scalar(0, 255, 0), -1); // 绿


    cv::imshow("fourPoint", image2show);
//    cv::waitKey(1);

//        for(int i = 0; i < 4; i++)
//            std::cout << vertices[i] << std::endl; // 四点顺序为 从左下角开始顺时针
//        std::cout << std::endl;
...
float AngleFactory::newtonIterationMethod() {
...
//    float resPitchAngle = Stm32Cmd.pitchData.float_temp - (float)atan(-y / z) * 180.0f / PI;    //期望角度赋初值，后面要不断迭代来抵消重力影响
    float resPitchAngle = mcu_data.curr_pitch - (float)atan(-y / z) * 180.0f / PI;    //期望角度赋初值，后面要不断迭代来抵消重力影响
    float funtionValue;
    float derivativeValue;
    unsigned int cnt = 0;
    float temp = 1.0f;  //牛顿迭代法中每次迭代减去的值，我们在这姑且叫它“余项”

//    float v = mcu_data.shoot_speed;
    float v = 22;


//    float alpha = Stm32Cmd.pitchData.float_temp;    //α以向上旋转为正值，为云台Pitch轴当前角度（水平为0度）
    float alpha = mcu_data.curr_pitch;    //α以向上旋转为正值，为云台Pitch轴当前角度（水平为0度）
...
void AngleFactory::getAngleCalculateData(TX2CmdStruct* TX2Cmd) {

    ArmorCoordinateSolver::CoordinateStruct coordinatePredict;
    coordinatePredict = _coordinateStruct;
    static float lastYawData = mcu_data.curr_yaw;
    static float lastPitchData = mcu_data.curr_pitch;
    static float lastYawRef = mcu_data.curr_yaw;
    static float lastPitchRef = mcu_data.curr_pitch;
    ...
    if (deltaTime_ms > MAX_TIME_BIAS) {
        lastYawData = mcu_data.curr_yaw;
        lastPitchData = mcu_data.curr_pitch;
        lastYawRef = mcu_data.curr_yaw;
        lastPitchRef = mcu_data.curr_pitch;
    }

    //检测角度数据的合法性
    float yawData, pitchData;
    if (isnan(mcu_data.curr_yaw)) {
        yawData = lastYawData;
        //LOG::debug("yawData is nan!");
        cout << "yawData is nan!" << endl;
    }
    else if (fabs(mcu_data.curr_yaw - lastYawData) > 45.0f) {
        yawData = lastYawData;
        //LOG::debug("yawData is abnormal!");
        //cout << "yawData is abnormal!" << endl;
    }
    else {
        yawData = mcu_data.curr_yaw;
        lastYawData = yawData;
    }

    if (isnan(mcu_data.curr_pitch)) {
        pitchData = lastPitchData;
        //LOG::debug("pitchData is nan!");
        cout << "pitchData is nan!" << endl;
    }
    else if (fabs((mcu_data.curr_pitch - lastPitchData) > 45.0f)) {
        pitchData = lastPitchData;
        //LOG::debug("pitchData is abnormal!");
        //cout << "pitchData is abnormal!" << endl;
    }
    else {
        pitchData = mcu_data.curr_pitch;
        lastPitchData = pitchData;
    }
//    cout <<"结果： "<< coordinatePredict.actualCoordinate<< endl;
    //获得处理后的陀螺仪角度期望
    float yawResult = getYawAngle(coordinatePredict.actualCoordinate.x, coordinatePredict.actualCoordinate.z, yawData+1);
//    cout <<yawResult << endl;
    float pitchResult;
    //英雄大弹丸要考虑空气阻力
    if (_carType == OLD_HERO || _carType == NEW_HERO) {
        pitchResult = airResistCalculation(22, pitchData, coordinatePredict.actualCoordinate);
    }
    else {
        pitchResult = equationCalculation(22, pitchData, coordinatePredict.actualCoordinate);
    }
    ...
    if (CurveData::isEnable) {
        ...
        float pitchNoBias = equationCalculation(22, pitchData, coordinateNoBias);
        CurveData::saveAngle(cv::Point2f(yawNoBias, pitchNoBias), cv::Point2f(yawResult, pitchResult), \
            cv::Point2f(mcu_data.curr_yaw, mcu_data.curr_pitch));
    }

//    cout << "yawBias:" << TX2Cmd->yawData.float_temp - mcu_data.curr_yaw << endl;
