...
public:
    ...
    void sendSmallEnergy();// 发送小能量机关数据
    void sendBigEnergy();  // 发送大能量机关数据
...
private:
    ...
    float target_distance;//步兵与待击打装甲板平面距离
    ...
    cv::Point3f predict_worldPTZ;//预测的world击打点坐标
    ...
    void showPredict(std::string windows_name, const cv::Mat &src);//显示预测打击点
    ...
    void getPredictPoint(cv::Mat src, cv::Point target_point);//获取预测点位
